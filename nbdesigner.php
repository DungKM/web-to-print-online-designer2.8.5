...
if (is_multisite()) {
    ...
    if (
        in_array('wordpress-mu-domain-mapping/domain_mapping.php', apply_filters('active_plugins', get_option('active_plugins')))
...
function nbdesigner_delete_background()
{
    ...
    if (
        !wp_verify_nonce($_POST['nonce'], 'nbdesigner_add_cat') ||
...
add_action('admin_head', function () {
    $plugin_pages = [
        ...
        'nbd_printing_options',
        'nbdesigner_get_quote',
        'nbd_template_mapping',
        ...
        if (isset($wp_filter['admin_notices']) && is_a($wp_filter['admin_notices'], 'WP_Hook')) {
            foreach ($wp_filter['admin_notices']->callbacks as $priority => $hooks) {
                foreach ($hooks as $hook_key => $hook_data) {
                    if (strpos($hook_key, 'nbdesigner') === false) {
                        remove_action('admin_notices', $hook_data['function'], $priority);
                    }
                }
            }
        }
