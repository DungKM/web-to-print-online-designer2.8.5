      ...
      switch (type) {
        case "flaticon":
          function getFlaticon(){
            $http({
               method: "GET",
               url: 'https://reprotel.com/wp-content/plugins/web-to-print-online-designer/proxy-api.php?page=' + $scope.resource[type].filter.currentPage + '&query=' + encodeURIComponent(search),
...
nbdApp.controller('ChatController', function($scope, $http, $timeout) {
  $scope.prompt = "";
  $scope.isLoading = false;
  $scope.responses = [];
  const OPENAI_API_KEY = NBDESIGNCONFIG["nbdesigner_open_ai_api_key"];
  ...
  $scope.generateResponse = function() {
      ...
      $http({
          ...
          data: {
              model: "gpt-3.5-turbo",
              messages: [{ role: "user", content: $scope.prompt }],
              max_tokens: 400,
              temperature: 0.7
          }
      }).then(function(response) {
          const botResponse = response.data.choices[0].message.content.trim();
          $scope.responses.push(botResponse);
          quill.setText($scope.responses.join('\n\n'));
